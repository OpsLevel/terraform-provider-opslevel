...
run "resource_filter_with_domain_id_predicate_equals" {
  ...
  variables {
    predicates = [
      for predicate_type in var.predicate_types_equals : {
        ...
        type     = predicate_type,
        key_data = null,
        value    = run.from_data_module.first_domain.id
      }
    ]
  }
  ...
  assert {
    condition = alltrue([
      opslevel_filter.this.predicate[0].value == run.from_data_module.first_domain.id,
      opslevel_filter.this.predicate[1].value == run.from_data_module.first_domain.id
    ])
    error_message = format(
      "expected predicate values to all be '%v' got values '%v'",
      run.from_data_module.first_domain.id,
